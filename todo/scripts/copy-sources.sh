...
if test `echo $ARCHETYPE_ARTIFACT_ID | grep multi`;then
  ...
  case "${ARCHETYPE_ARTIFACT_ID}" in
    *mybatis2* )
      cp -p -r ../src-mybatis2/main/java/com/example/todo/domain ./${ARTIFACT_ID}/${ARTIFACT_ID}-domain/src/main/java/com/example/todo
      cp -p -r ../src-mybatis2/main/resources/META-INF/mybatis ./${ARTIFACT_ID}/${ARTIFACT_ID}-domain/src/main/resources/META-INF
      ;;
    *jpa* )
  ...
  case "${ARCHETYPE_ARTIFACT_ID}" in
    *mybatis2* )
      cp -p -r ../src-mybatis2/* ./${ARTIFACT_ID}/src
      ;;
    *jpa* )
